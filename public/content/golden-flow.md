...
I owned design across Dream11's core fantasy journey (home to live tracking) over multiple years and IPL seasons, for a product used by 250M+ users. I started as a Front-End Developer on Dream11's Progressive Web App, moving into design just a year and a half into my role. Every screen and feature here shipped as a scoped iteration, not a single planned redesign, since a flow at this scale can't absorb a big-bang rebuild.
...
The primary discovery screen and first decision point when a user opens the app. Iterated the home screen and match card design across multiple versions over several seasons, expanding from a single-sport match list to a multi-sport one, while refining visual density without adding clutter.

Also housed the DreamSale campaign, "Is Hafte Naya Kya?", an 11-week themed weekly-offer structure. Outcome not independently measured, since it coincides with peak IPL traffic and usage shifts can't be isolated to the redesign.
&nbsp;
...
Primary screen driving contest joins and the core revenue decision point in the app. Evolved through incremental aesthetic refinements over several years, with no full overhaul. Outcome not independently measured.
&nbsp;
